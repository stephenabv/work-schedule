...
1. Copy `.env.example` to `.env.local` for local dev, or set the same keys directly in your Vercel project's Environment Variables (Settings → Environment Variables) for deployed use:
   ...
   - `SEMAPHORE_SENDER_NAME` — optional, a Semaphore-registered sender name (defaults to `AzariSolar`). The name must be registered **and approved** on the Semaphore account, or Semaphore rejects the message.
   - `NOTIFY_PHONE_NUMBER` — the PH-format number(s) that receive reminders: one number (`09171234567`) or several separated by commas (`09171234567,09181234567`).
